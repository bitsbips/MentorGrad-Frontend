...
const TestDashboard: FC = () => {
    ...
    const list = (
        <div role="presentation" onClick={toggleDrawer(false)} onKeyDown={toggleDrawer(false)}>
            <List>
                {Data.map((data) => (
                    <ListItem key={data.id} onClick={() => [SetStatusfunc(data.name), data.id === 1 ? setProfileStep('0') : data.id === 2 ? setProfileStep('1') : data.id === 3 ? setProfileStep('2') : data.id === 4 ? setProfileStep('3') : setProfileStep('4')]}>
                        {Status === data.name ?
                            <>
                                <BackActive>
                                    {data.icon && <data.icon style={{ color: '#fff', fontSize: '20px' }} />}
                                </BackActive>
                ...
                {isMobile ? (
                    ...
                    <div style={style}>
                        ...
                        {Data.map((data) => {
                            return (
                                <>
                                    <div onClick={() => [SetStatusfunc(data.name), data.id === 1 ? setProfileStep('0') : data.id === 2 ? setProfileStep('1') : data.id === 3 ? setProfileStep('2') : data.id === 4 ? setProfileStep('3') : setProfileStep('4')]}>
                                        {Status === data.name ?
                                            <div style={{ display: 'flex', flexDirection: 'row' }}>
                                                <BackActive>
                                                    {data.icon && <data.icon style={{ color: '#fff', fontSize: '20px' }} />}
                    ...
                    <>
                        ...
                        <div onClick={toggleDrawer(true)} style={{alignSelf:'flex-start',marginLeft:"-5%",marginBottom:'3%'}}>
