...
const Conversation = () => {
  ...
  const [fetchPrivateMsgs, { data: privateData, loading: loadingPrivate }] =
    useLazyQuery(GET_PRIVATE_MSGS, {
      onError: (err) => {
        notifyError(getErrorMsg(err));
      },
    });
  const [fetchGroupMsgs, { data: groupData, loading: loadingGroup }] =
    useLazyQuery(GET_GROUP_MSGS, {
      onError: (err) => {
        notifyError(getErrorMsg(err));
      },
    });
  const [fetchGlobalMsgs, { data: globalData, loading: loadingGlobal }] =
  ...
  const { error: subscriptionError } = useSubscription(NEW_MESSAGE, {
    onSubscriptionData: ({ client, subscriptionData }) => {
      ...
      let _lastMsg =
        conversationCache?.getPrivateMessages[
          conversationCache?.getPrivateMessages?.length - 1
        ];
      if (
        newMessage?.message?.body === _lastMsg?.body &&
        newMessage?.message?.createdAt === _lastMsg?.createdAt
      ) {
        return;
      }
  ...
  useEffect(() => {
    ...
    if (
      selectedChat.chatData.latestMessage &&
      ...
      changeMsgSeenStatus({
        ...
        update: (proxy, { data }) => {
          ...
          if (conversationCache) {
            ...
            if (lastMsgCache) {
              const updatedLastMsgCache = lastMsgCache[getLastMsgQueryName].map(
                (l) =>
                  l.id === lastMsgTargetId
      ...
      return (
        <div className={classes.conversationWrapper}>
          {messages.map((message, index) => {
            ...
            //Message Seen By
            const messageSeenBy = message.seenBy.filter(
              (seen) => seen.participant != user.id
            );
                  ...
                  {isGroupGlobalChat &&
                    ...
                    user.id !== message.senderId && (
                      <Typography
                        ...
                        color="secondary"
                        onClick={() => selectChat(message.user, "private")}
                      >
  ...
  return (
    <div className={classes.root} ref={ref}>
      ...
      <SendMessage
        capture={handleClickTakeScreenShot}
        selectedShots={state.selectedShots}
      />
